...
| `NEXT_PUBLIC_SHOW_USAGE` | boolean-ish string (optional) | unset (**on**) | Per-reply usage/metrics footer (time · TTFT · tokens · cost) + per-tool run-time. Opt-**out**: `"0"`/`"false"`/`"no"`/`"off"` (case-insensitive) ⇒ hidden; anything else / unset ⇒ shown. Time & TTFT are measured client-side; tokens/cost/tool-time come from the backend `usage` frame (see [`chat-transport.md`](../../../docs/design-docs/chat-transport.md)). |
