...
export function streamReplay(options: StreamReplayOptions): ReplayHandle {
  ...
  // Clamped, mutable timing. Base delays are read per-frame; speed can change live.
  let textDelayMs = clamp(options.timing?.textDelayMs ?? TEXT_DELAY_MS, 0, MAX_DELAY_MS);
  let toolDelayMs = clamp(options.timing?.toolDelayMs ?? TOOL_DELAY_MS, 0, MAX_DELAY_MS);
